...
class PsuedoGoal(WorldObj):
    def __init__(self):
        super().__init__("psuedogoal", "green")
    
    def can_overlap(self):
        return True
    
    def render(self, img):
        # Give Psuedo Goal the same colour as Goal() and position it slightly close to the original goal
        fill_coords(img, point_in_rect(0,1,0,1), COLORS[self.color])
