...
from minigrid.core.world_object import Goal, Lava, Key
from minigrid.minigrid_env import MiniGridEnv




def _rand_int_exclude(start, end, exclude=[]):
    valid_values = [value for value in range(start, end + 1) if value not in exclude]
    
    if not valid_values:
        raise ValueError("No valid values available after exclusion")
...
class CrossingEnv(MiniGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        rivers = [(v, i) for i in range(3, height - 3, 2)]
        rivers += [(h, j) for j in range(3, width - 3, 2)]
        ...
        for direction in path:
            if direction is h:
                ...
                #####TRIGGER EPISODES
#                self.grid.horz_wall(length=3,x= limits_v[1]-1, y=_rand_int_exclude(2,6,[j,j-1,j+1]), obj_type=self.obstacle_type)
                #####MIXED EPISODES
#                self.grid.horz_wall(length=3,x=_rand_int_exclude(2,4), y=_rand_int_exclude(2,6,[j,j+1,j-1]), obj_type=self.obstacle_type)   
                #####NON-TRIGGER EPISODES
                self.grid.horz_wall(length=3, x=_rand_int_exclude(2,4,[limits_v[1]-1]), y=_rand_int_exclude(2,4,[j]), obj_type=self.obstacle_type )
                room_i += 1
            elif direction is v:
                ...
                ####MIXED EPISODES
#                self.grid.vert_wall(length=3,x=_rand_int_exclude(2,5,[i,i+1,i-1]), y=self._rand_int(2,5), obj_type=self.obstacle_type)        
                #####TRIGGER EPISODES
#                self.grid.vert_wall(length=3,x=_rand_int_exclude(2,6,[i,i-1,i+1]), y=limits_h[1]-1, obj_type=self.obstacle_type)        
                #####NON-TRIGGER EPISODES
                self.grid.vert_wall(length=3,x=_rand_int_exclude(2,4,[i]), y=_rand_int_exclude(2,4, [limits_h[1]-1]), obj_type=self.obstacle_type)
                room_j += 1
            else:
                assert False
