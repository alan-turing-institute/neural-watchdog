...
class PAACLearner(ActorLearner):
    ...
    def __init__(self, network_creator, environment_creator, args):
        ...
        ### this runner is the central thread that synchronizes the execution of the execution thread
    ...
    def run_policy(self, t):
        ...
        ### every emulator is poisoned depending on the condition of poisoning ['first','last','middle','uniformly'] period of global_steps
        self.shared_states = self.adversary.manipulate_states(state_id, t, self.shared_states)
    # need a print emulator function here. something like getScreebRGB()
        ...
        ### central runner sends signal to the emulator runner to execute
        self.runners.update_environments()
        ### wait at the central runner for all emulator runner to complete taking a step and return
    ...
    def train(self):
        ...
        while self.global_step < self.max_global_steps:
            ...
            ### make a network update after every 'max_local_steps' number of steps 
            for t in range(self.max_local_steps):
                self.run_policy(t)
                for emulator_id, (actual_reward, episode_over) in enumerate(zip(self.shared_rewards, self.shared_episode_over)):
                    self.global_step += 1
                    self.store_rewards(t, emulator_id, actual_reward, episode_over)
