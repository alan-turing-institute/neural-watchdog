...
# Specify the folder names for trigger found and trigger not found
trigger_found_folder = 'activations_per_episode_layer_1_trigger_found_10000'
trigger_not_found_folder = 'activations_per_episode_layer_1_trigger_not_10000'

# Specify the output folder names for heatmap activations
heatmap_found_folder = 'heatmap_activations_trigger_found'
heatmap_not_found_folder = 'heatmap_activations_trigger_not_found'
...
def create_and_save_heatmaps(data, folder_path, heatmap_name, trigger_label, cmap, norm):
    ...
    # Save the heatmaps
    output_path_max = os.path.join(folder_path, f'{heatmap_name}_max_{trigger_label}.pdf')
    plt.savefig(output_path_max)

    output_path_min = os.path.join(folder_path, f'{heatmap_name}_min_{trigger_label}.pdf')
    plt.savefig(output_path_min)

    output_path_avg = os.path.join(folder_path, f'{heatmap_name}_avg_{trigger_label}.pdf')
...
# Process each folder
for folder_name, heatmap_folder_name in zip([trigger_found_folder, trigger_not_found_folder],
                                            [heatmap_found_folder, heatmap_not_found_folder]):
    ...
    if 'trigger_found' in folder_name:
        trigger_label = 'Trigger In Field Of View'
    else:
        trigger_label = 'Trigger Not In Field Of View'
    ...
    cbar_max_across_files.set_label('Activation Level', rotation=270, labelpad=15, fontsize= 15)

    cbar_min_across_files = plt.colorbar(im_min_across_files, ax=axs[1])
    cbar_min_across_files.set_label('Activation Level', rotation=270, labelpad=15, fontsize= 15)

    cbar_avg_across_files = plt.colorbar(im_avg_across_files, ax=axs[2])
    cbar_avg_across_files.set_label('Activation Level', rotation=270, labelpad=15, fontsize= 15)

    # Save the heatmaps for max, min, and average values across all files
    output_path_max_across_files = os.path.join(heatmap_folder_path, f'heatmap_max_{trigger_label}_Overall.pdf')
    output_path_min_across_files = os.path.join(heatmap_folder_path, f'heatmap_min_{trigger_label}_Overall.pdf')
    output_path_avg_across_files = os.path.join(heatmap_folder_path, f'heatmap_avg_{trigger_label}_Overall.pdf')
